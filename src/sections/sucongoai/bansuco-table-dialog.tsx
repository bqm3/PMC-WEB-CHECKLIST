...
export default function BansucoDialog({ isMenu = false }: BansucoDialogProps) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://checklist.pmcweb.vn/be/api/v2/bansuco');
    ...
    try {
      if (formMode === 'add') {
        const response = await axios.post(
          'https://checklist.pmcweb.vn/be/api/v2/bansuco/create',
          currentEmployee
        );
        if (response.data.success) {
          setSnackbar({
        ...
        const response = await axios.put(
          `https://checklist.pmcweb.vn/be/api/v2/bansuco/update/${currentEmployee.id}`,
          currentEmployee
        );
        if (response.data.success) {
          setSnackbar({
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await axios.put(
        `https://checklist.pmcweb.vn/be/api/v2/bansuco/delete/${employeeToDelete.id}`
      );
      if (response.data.success) {
        setSnackbar({
          open: true,
