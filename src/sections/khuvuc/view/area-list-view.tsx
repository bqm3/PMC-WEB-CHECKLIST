...
export default function AreaListView() {
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    await axios
      .put(`${process.env.REACT_APP_HOST_API}/api/v2/ent_khuvuc/delete-mul`, deleteRows, {
        headers: {
          Accept: 'application/json',
          Authorization: `Bearer ${accessToken}`,
        },
  ...
  const handleDownloadImages = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_HOST_API}/api/v2/ent_khuvuc/generate-qr-codes`,
        {
...
function applyFilter({
  ...
  if (name) {
    inputData = inputData.filter(
      (order) =>
        `${order.Tenkhuvuc}`?.normalize('NFC')?.toLowerCase().indexOf(name?.normalize('NFC')?.toLowerCase()) !== -1 ||
        `${order.MaQrCode}`?.normalize('NFC')?.toLowerCase().indexOf(name?.normalize('NFC')?.toLowerCase()) !== -1 ||
        `${order.ent_toanha.Toanha}`?.normalize('NFC')?.toLowerCase().indexOf(name?.normalize('NFC')?.toLowerCase()) !== -1
    );
  }

  if (building.length) {
    inputData = inputData.filter((item) => building.includes(String(item?.ent_toanha?.ID_Toanha)));
  }
